...
if search_parameters
    ...
    lmnn_par.threshold = linspace(0.1,2.5,lmnn_par.n_thresholds)';

    % KLMNN
    klmnn_par.n_thresholds = 20;
    klmnn_par.threshold = linspace(0.1,3.5,klmnn_par.n_thresholds)';
    ...
    knfst_par.threshold = linspace(0.01,0.2,knfst_par.n_thresholds)';
    ...
    one_svm_par.kernel = linspace(1.0,3.0,one_svm_par.n_kernels)';    
    ...
    multi_svm_par.kernel = linspace(5.0,10.0,multi_svm_par.n_kernels)';

    % KPCA
    kpca_par.n_thresholds = 20;
    kpca_par.threshold = linspace(0.1,1.0,kpca_par.n_thresholds)';
    kpca_par.kernel_type = 'poly'; % offset = 1.0;
    kpca_par.n_kernels = 10;
    kpca_par.kernel = linspace(1,10,kpca_par.n_kernels)';    
    ...
    %Methods.runValidations(X,y,methods([7]),parameters,out_dir,n_experiments,...
    ...
    %Methods.runValidationsForKnnMethods(X,y,methods([1,2,3]),parameters,out_dir,n_experiments,...
    %                       n_classes,untrained_classes,training_ratio,view_plot_metric)

    %% AVALIAÇÃO
    % Avalia os métodos e salva as métricas de acurácia
    %Methods.runEvaluations(xtrain,ytrain,xtest,ytest,methods([7]),out_dir,n_classes,K,kappa);
    
    % Avalia os métodos knn, lmnn, klmnn e salva as métricas de acurácia
    %Methods.runEvaluationsForKnnMethods(xtrain,ytrain,xtest,ytest,methods([1,2,3]),out_dir,n_experiments);
        
    % Executa a predição dos métodos em pontos do grid e desenha as fronteiras das classes
    Methods.runPredictions(X,y,xtest,methods([1,2,3,4,5,6,7]),out_dir,n_classes,K,kappa);
    ...
    klmnn_par.kernel_type = 'poly'; % ou 'gauss';
    klmnn_par.kernel_arg = 2.0;

    % KNFST
    knfst_par.threshold_arg = 0.05;
    knfst_par.kernel_type = 'poly';% ou 'gauss'
    ...
    Methods.runPredictionsParameter(xtrain,ytrain,xtest,methods([1]),parameters,out_dir,n_classes,K,kappa);
end
